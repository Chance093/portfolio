...
class Terminal {
    ...
    computeDelay(log) {
        if (["Loading drivers", "Mounting virtual", "Starting system"].some((tag) => log.includes(tag))) {
            this.bootDelay *= 8;
        }
        if (["Starting udev", "devpts", "Cleaning up"].some((tag) => log.includes(tag))) {
            this.bootDelay /= 8;
        }
    }
    renderAscii() {
...
class Command {
    ...
    exec404() {
        const p = createElement("p", ["output"], `${this.command}: command not found`);
